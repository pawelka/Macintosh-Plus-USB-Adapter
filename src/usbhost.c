...
* File Name          : USBHOST.C
* Author             : pkarpins / WCH
* Version            : V1.2 - support for multiple USB devices
* Date               : 2018/02/28 (WCH) / 2024/06/04 (pkarpins)
...
uint8_t AnalyzeHidInterfaces( __xdata uint8_t *buf )
{
    uint8_t i, l, total, found;
    uint8_t if_num, if_cls, if_subcls, if_proto;
    uint8_t ep;
    ...
    if_subcls = 0;
    if_proto = 0;
    total = ( (PXUSB_CFG_DESCR)buf ) -> wTotalLengthL;

    ThisUsbDev.DeviceTypes = 0;
    ThisUsbDev.KeyboardEndp = 0;
    ThisUsbDev.MouseEndp = 0;
    ThisUsbDev.KeyboardInterface = 0;
    ThisUsbDev.MouseInterface = 0;
    ...
    while ( i < total )
    {
        ...
        if ( buf[i+1] == USB_DESCR_TYP_INTERF )
        {
            if_num = ( (PXUSB_ITF_DESCR)(buf+i) ) -> bInterfaceNumber;
            if_cls = ( (PXUSB_ITF_DESCR)(buf+i) ) -> bInterfaceClass;
            if_subcls = ( (PXUSB_ITF_DESCR)(buf+i) ) -> bInterfaceSubClass;
            if_proto = ( (PXUSB_ITF_DESCR)(buf+i) ) -> bInterfaceProtocol;
#if DEBUG_SERIAL
            printstr("IF ");printx2(if_num);
            printstr(" cls ");printx2(if_cls);
            printstr(" sub ");printx2(if_subcls);
            printstr(" proto ");printx2(if_proto);printlf();
#endif
        }
        else if ( buf[i+1] == USB_DESCR_TYP_ENDP
                && if_cls == USB_DEV_CLASS_HID
                && ( ( (PXUSB_ENDP_DESCR)(buf+i) ) -> bmAttributes & USB_ENDP_TYPE_MASK ) == USB_ENDP_TYPE_INTER
                && ( ( (PXUSB_ENDP_DESCR)(buf+i) ) -> bEndpointAddress & USB_ENDP_DIR_MASK ) )
        {
            ep = ( (PXUSB_ENDP_DESCR)(buf+i) ) -> bEndpointAddress & USB_ENDP_ADDR_MASK;

            if ( if_subcls <= 1 && if_proto == 1 && ThisUsbDev.KeyboardEndp == 0 )
            {
                ThisUsbDev.KeyboardEndp = ep;
                ThisUsbDev.KeyboardInterface = if_num;
                ThisUsbDev.DeviceTypes |= DEV_HAS_KEYBOARD;
                ThisUsbDev.GpVar[0] = ep;
                found++;
#if DEBUG_SERIAL
                printstr("KBD ep ");printx2(ep);printlf();
#endif
            }
            else if ( if_subcls <= 1 && if_proto == 2 && ThisUsbDev.MouseEndp == 0 )
            {
                ThisUsbDev.MouseEndp = ep;
                ThisUsbDev.MouseInterface = if_num;
                ThisUsbDev.DeviceTypes |= DEV_HAS_MOUSE;
                ThisUsbDev.GpVar[1] = ep;
                found++;
#if DEBUG_SERIAL
                printstr("MOUSE ep ");printx2(ep);printlf();
#endif
            }
        }
...
{
    uint8_t   t,i, s, cfg, dv_cls, if_cls;
    t = 0;
#if DEBUG_SERIAL	
    ...
    if ( s == ERR_SUCCESS )
    {
        ...
        if ( s == ERR_SUCCESS )
        {
            ...
            if ( s == ERR_SUCCESS )
            {
                ...
                s = AnalyzeHidInterfaces( Com_Buffer );
                if ( s )
                {
#if DEBUG_SERIAL
                    printstr( "AnalyzeHidInterfaces " );printhex2(s);
                    printstr(" flags ");printhex2(ThisUsbDev.DeviceTypes);printlf();
                    printstr( "CtrlSetUsbConfig " );printhex2(cfg);printlf();
#endif
                    ...
                    if ( s == ERR_SUCCESS )
                    {
                        ...
                        {
                            CtrlGetHIDDeviceReport( ThisUsbDev.KeyboardInterface );
                            SetBootProto( ThisUsbDev.KeyboardInterface );
#if DEBUG_SERIAL
                            printstr( "USB-Keyboard Ready IF " );
                            printx2(ThisUsbDev.KeyboardInterface);
                            printstr(" EP ");printx2(ThisUsbDev.KeyboardEndp);printlf();
#endif
                        }

                        if ( ThisUsbDev.DeviceTypes & DEV_HAS_MOUSE )
                        {
                            CtrlGetHIDDeviceReport( ThisUsbDev.MouseInterface );
                            SetBootProto( ThisUsbDev.MouseInterface );
#if DEBUG_SERIAL
                            printstr( "USB-Mouse Ready IF " );
                            printx2(ThisUsbDev.MouseInterface);
                            printstr(" EP ");printx2(ThisUsbDev.MouseEndp);printlf();
#endif
                        }

                        if ( ThisUsbDev.DeviceTypes & DEV_HAS_MOUSE )
                        {
                            ThisUsbDev.DeviceType = ( ThisUsbDev.DeviceTypes & DEV_HAS_KEYBOARD ) ? DEV_TYPE_MOUSE2 : DEV_TYPE_MOUSE;
                        }
                        else if ( ThisUsbDev.DeviceTypes & DEV_HAS_KEYBOARD )
                        {
...
uint16_t  SearchTypeDevice( uint8_t type )   
{
	...
	if ( ( type == DEV_TYPE_KEYBOARD ) && ( ThisUsbDev.DeviceTypes & DEV_HAS_KEYBOARD ) )
	{
		return( (uint16_t)RootHubIndex << 8 );
	}
	if ( ( ( type == DEV_TYPE_MOUSE ) || ( type == DEV_TYPE_MOUSE2 ) ) && ( ThisUsbDev.DeviceTypes & DEV_HAS_MOUSE ) )
	{
		return( (uint16_t)RootHubIndex << 8 );
	}
