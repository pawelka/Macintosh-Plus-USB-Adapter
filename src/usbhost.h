...
#define DEV_HAS_KEYBOARD    0x01
#define DEV_HAS_MOUSE       0x02
...
typedef struct
{
    ...
    uint8_t   DeviceTypes;               // HID capability flags for composite devices
	uint16_t  DeviceVID;
	uint16_t  DevicePID;
    uint8_t   GpVar[4];                    // 通用变量，存放端点
    uint8_t   KeyboardEndp;
    uint8_t   MouseEndp;
...
extern __idata _RootHubDev ThisUsbDev;
